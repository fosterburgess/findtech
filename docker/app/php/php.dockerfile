...
RUN apt-get install -y procps htop vim zip unzip libzip-dev libicu-dev libpq-dev libpng-dev git curl
RUN NODE_MAJOR=20 apt-get install -y nodejs npm;

# install php docker extensions
RUN \
  docker-php-ext-configure intl && \
  docker-php-ext-install intl pcntl zip pdo pdo_pgsql pdo_mysql gd opcache exif
